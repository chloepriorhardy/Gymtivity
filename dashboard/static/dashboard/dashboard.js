...
const newFriendsRequest = new Request("/api/makefriends/", {
  method: "GET",
  headers: { "X-CSRFToken": csrf_token },
});
...
function handleNewFriendData(data) {
  // The list group element that items will get appended to upon processing data
  // retrieved from our `friends` API endpoint.
  const friendsList = document.getElementById("newFriendList");
...
const newFriendsModalEl = document.getElementById("newFriendsModal");
newFriendsModalEl.addEventListener("show.bs.modal", function (event) {
  fetch(newFriendsRequest)
    .then((response) => {
      if (response.status === 200) {
        response.json().then(handleNewFriendData);
      } else {
        throw new Error("Something went wrong on api server!");
      }
    })
